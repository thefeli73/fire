...
describe('buildSitemapEntries', () => {
  it('includes known static routes', async () => {
    ...
    const urls: string[] = sitemap.map((entry: (typeof sitemap)[number]) => entry.url);
    ...
    expect(urls).toContain('https://investingfire.com/learn/cagr-calculator');
    expect(sitemap.every((entry: (typeof sitemap)[number]) => entry.lastModified instanceof Date)).toBe(
      true,
    );
  });
